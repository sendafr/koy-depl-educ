...
function Media() {
  ...
  const fetchMediaCategory = async () => {
    try {
      const response = await mediaCategoryAPI.getAll();
      setMediaCategory(response.data.data || []);
    } catch (err) {
      console.error('Failed to fetch categories:', err);
    }
  };

  const fetchMediaTag = async () => {
    try {
      const response = await mediaTagAPI.getAll();
      setMediaTag(response.data.data || []);
    } catch (err) {
      console.error('Failed to fetch mediaTags:', err);
    }
  };
  ...
  const handleMediaTagToggle = (mediaTagId) => {
    setFormData((prev) => ({
      ...prev,
      mediaTag_ids: prev.mediaTag_ids.includes(mediaTagId)
        ? prev.mediaTag_ids.filter((id) => id !== mediaTagId)
        : [...prev.mediaTag_ids, mediaTagId],
    }));
  };
        ...
        <div style={{ width: '100%', backgroundColor: '#000' }}>
          <video
            ...
            onError={(e) => {
              ...
              console.error('Video src:', previewSrc);
            }}
          />
        </div>
      );
    ...
    case 'document':
      return (
        <div className="modal-preview-document" style={{ textAlign: 'center', padding: '2rem' }}>
          ...
          <a href={previewSrc} target="_blank" rel="noopener noreferrer" className="btn btn-primary" style={{ marginTop: '1rem', display: 'inline-block' }}>
            Open Document
          </a>
